...
async def fetch_curr_price(exchange, ticker):
    price = (
        exchange.fetch_ticker(ticker)["ask"] + exchange.fetch_ticker(ticker)["bid"] / 2
    )
    return price
...
async def withdraw_function(
    ...
    try:
        ...
        raise HTTPException(
            code=500, detail="DB error cannot get the withdraw address from database"
        )
...
async def buy_function(response, websocket, uid, exchange, symbol, price):
    ...
    try:
        current_price = (
            exchange.fetch_ticker(symbol)["ask"]
            + exchange.fetch_ticker(symbol)["bid"] / 2
        )
        amount = price / current_price
