...
async def withdraw_function(response, websocket, uid, last_sell, initial_buy):
    ...
    await websocket.send(
        f"{{Message : TOTAL P&L : {last_sell - initial_buy}, uid : {uid}}}"
    )
...
async def sold(
    response, count, number_of_trades, initial_buy, last_sell, websocket, uid
):
...
class TestBot:
    async def test(
        self, loss=None, profit=None, number_of_trades=None, uid=None, ticker=None
    ):
            ...
            async with websockets.connect("ws://localhost:8765") as websocket:
                ...
                if flag and buyed:
                    stop_loss, profit_margin = await check(
                        stop_loss=loss, buy=buyprice, profit=profit
                    )
...
async def test_generator(
    ...
    try:
        Process(
            target=TestBot().process,
            args=(exchange, loss, profit, total_number_of_trades, uid, ticker),
        ).start()
